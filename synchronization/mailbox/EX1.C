...
OS_EVENT     *RandomSem;
OS_EVENT     *MailBox; // DecisionTask와 Task1 과의 MailBox
OS_EVENT     *MailBox2; // DecisionTask와 Task2 과의 MailBox
OS_EVENT     *MailBox3; // DecisionTask와 Task3 과의 MailBox
OS_EVENT     *MailBox4; // DecisionTask와 Task4 과의 MailBox
...
void  TaskStart (void *pdata)
{
	...
	MailBox3 = OSMboxCreate((void *)0);
	MailBox4 = OSMboxCreate((void *)0);
...
void  DecisionTask (void *pdata)
{
	...
	INT8U err, i, sel, res;
	INT8U num[4] = {0,0,0,0};
	INT8U min = 65; // minimum value
	char win = 'W', lose = 'L'; // RandomTask로 보낼 문자
	
	// For compiler warning
	pdata = pdata;
	
	for(;;){		
		// MailBox로부터 넘어온 데이터를 받습니다.		
		INT8U *data1 = (INT8U *) OSMboxPend(MailBox,0,&err);
		INT8U *data2 = (INT8U *) OSMboxPend(MailBox2,0,&err);
		INT8U *data3 = (INT8U *) OSMboxPend(MailBox3,0,&err);
		INT8U *data4 = (INT8U *) OSMboxPend(MailBox4,0,&err);
		...
		// 가장 작은 숫자를 보낸 MailBox에만 'W'를 보내고, 나머지 메일박스에는 'L'을 보냅니다.
		switch(sel) {
		case 0:
			res = OSMboxPost(MailBox,(void *)&win);
			res = OSMboxPost(MailBox2,(void *)&lose);
			res = OSMboxPost(MailBox3,(void *)&lose);
			res = OSMboxPost(MailBox4,(void *)&lose);
			break;
		case 1:
			res = OSMboxPost(MailBox,(void *)&lose);
			res = OSMboxPost(MailBox2,(void *)&win);
			res = OSMboxPost(MailBox3,(void *)&lose);
			res = OSMboxPost(MailBox4,(void *)&lose);
			break;
		case 2:
			res = OSMboxPost(MailBox,(void *)&lose);
			res = OSMboxPost(MailBox2,(void *)&lose);
			res = OSMboxPost(MailBox3,(void *)&win);
			res = OSMboxPost(MailBox4,(void *)&lose);
			break;
		case 3:
			res = OSMboxPost(MailBox,(void *)&lose);
			res = OSMboxPost(MailBox2,(void *)&lose);
			res = OSMboxPost(MailBox3,(void *)&lose);
			res = OSMboxPost(MailBox4,(void *)&win);
			break;			
		}								
...
void RandomTask (void *pdata){
	...
	for(;;){
		...
		if (*data == '0') {
			res = OSMboxPost(MailBox,(void *)&rnd);					
			win = (char *)OSMboxPend(MailBox,0,&err);
			...
			res = OSMboxPost(MailBox2,(void *)&rnd);
			win = (char *)OSMboxPend(MailBox2,0,&err);			
			temp = rnd;
			// Task2 옆에 숫자 출력
			for (i = 0; i < 2; i++) {
				ch = (char) (temp % 10) + '0';
			...
			res = OSMboxPost(MailBox3,(void *)&rnd);
			win = (char *)OSMboxPend(MailBox3,0,&err);			
			temp = rnd;
			// Task3 옆에 숫자 출력
			for (i = 0; i < 2; i++) {
				ch = (char) (temp % 10) + '0';
			...
			res = OSMboxPost(MailBox4,(void *)&rnd);			
			win = (char *)OSMboxPend(MailBox4,0,&err);			
			temp = rnd;
			// Task4 옆에 숫자 출력
			for (i = 0; i < 2; i++) {
				ch = (char) (temp % 10) + '0';
